...
trait CommandTrait
{
            ...
            if (is_numeric($count)) {
                ...
                if (!empty($flags)) {
                    foreach (explode(' ', $flags) as $flag) {
                        ...
                        if (Helpers::haveNot($flag)) {
                            $flag_without_not = substr($flag, 0, -1);
                        }
                        if (in_array($flag_without_not, array_keys(Constants::LAST_FLAG))) {
                            $to_accept[] = $flag;
                        } else {
                            $fs .= __('flag_not_exist', ['flag' => $flag]);
                        }
                    }
                } else {
                    $to_accept = ['-all'];
    ...
    public function codeRunner(Message $message): bool
    {
        ...
        if (empty($code)) {
            return $this->answer(
                $chat,
                fe: __('code_not_found'),
                message_to_edit: $message_to_edit ?? $message
            );
        }
        ...
        try {
            ...
            if (isset($quote))
                $this->messages->sendMessage(peer: $chat, reply_to: $quote, message: __('bold', ['bold' => "Errors :\n"]) . $error, parse_mode: Constants::DefaultParseMode);
        } catch (\Throwable $e) {
    ...
    public function queryRunner(Message $message): bool
    {
        ...
        if ($this->databaseService !== null) {
            try {
                ...
                $fe = __('runner.results', ['data' => Helpers::queryResult2String($result)]);
            ...
            $fe = __('runner.without_database_connection');
        }
        return $this->answer($chat, $fs ?? null, $fe ?? null, $message_to_edit ?? $message, $reply2id ?? null);
    }
    ...
    public function downloaderUploader(Message $message): bool
    {
        ...
        if (preg_match('/^\/download(?:(?:\s([\w\.\s-]+))?)$/', $message_text, $matches)) {
            ...
            if (isset($this->settings['DL_SERVER_HOST']))
                $download_script_url = $this->settings['DL_SERVER_HOST'];
            if (empty($download_script_url))
                return $this->answer(peer: $chat, fe: __('dlUp.download_script_url_wrong'), message_to_edit: $message_to_edit ?? $message);

            $name ??= preg_replace("~^(.+)\_(\d+)\.(\w{2,5})$~", '$1.$3', $media->fileName);//separate file id from file name

            try {
                $downloads = $this->ormProperty['downloads'] ?? [];
                $id = Helpers::newItemWithRandomId(function ($uniq_id) use ($downloads) {
                ...
                $fe = str_contains($e->getMessage(), 'downloadServer(')
                    ? __('dlUp.download_script_url_wrong')
                    : $e->getMessage();
            }
            ...
            if (isset($url)) {
                ...
                $cb = function ($percent, $speed, $time) use ($message_to_edit) {
                    ...
                    $now = microtime(true);
                    if (($now - $prev_time <= 2 || $prev_percent == $percent) and (int)$percent !== 100)
                        return;//edit text every 2 sec and when percent was changed

                    $prev_time = $now;
                    $prev_percent = (int)$percent;

                    $text = $percent != 100
                        ? __('dlUp.upload_percent', ['percent' => (int)$percent, 'speed' => $speed, 'time' => $time])
                        : __('dlUp.upload_successfully', ['time' => $time, 'speed' => $speed]);
                    try {
                        $message_to_edit->editText($text, parseMode: Constants::DefaultParseMode);
    ...
    public function getLinkMessage(Message $message): bool
    {
        ...
        if (!isset($channel) or !isset($message_ids))
            return $this->answer($chat, fe: __('getmessagelink.bad_command'), message_to_edit: $message_to_edit ?? $message);
        try {
            ...
            foreach ($messages['messages'] as $message_item) {
                if (isset($message_item['media'])) {
                    ...
                    $this->sendMessage($chat, $message_item['message'], replyToMsgId: $reply2id);
                }
            }
        } catch (RPCErrorException $e) {
            $fe = $e->getMessage();
    ...
    public function saveStory(Message $message): bool
    {
        ...
        if (preg_match('#^\/savestory\shttps://t\.me/(\w+)/s/(\d+)$#', $message_text, $matches)) {
            ...
            if (isset($reply->fromId) and $reply->fromId != null) {
                ...
                $fe = __('story.cant_find_user_from_message');
            }
        ...
        if (isset($user_story)) {
            ...
            if (isset($story_id)) {
                ...
                $stories = $this->stories->getPeerStories(peer: $user_story);
            }
            if (!empty($stories['stories'])) {
                ...
                $fe = __('story.no_story_exist', ['tag' => $tag]);
            }
        }
    ...
    public function convertMedia(Message $message): bool
    {
        $message_text = $message->message;
        $chat = $message->chatId;
        if (!preg_match('~\/convert\s(?:to\s)?(video|gif|voice|audio|sticker|photo)~', $message_text, $matches))
            return false;
        if (is_null($reply = $message->getReply()))
            return $this->answer($chat, fs: __('must_reply_with_media'));
        if (!isset($reply->media))
            return $this->answer($chat, fs: __('replayed_no_media'));
    ...
    public function changeMediaAttrs(Message $message): bool
    {
        $chat = $message->chatId;
        $message_text = $message->message;
        if (!preg_match("/^\/change\s(filename|title|performer|mime|size)\sto\s(.+)$/sui", $message_text, $matches))
            return false;
        if (null === $reply = $message->getReply()) {
            return $this->answer($chat, fs: __('must_reply_with_media'));
        }
        if (null === $media = $reply->media) {
            return $this->answer($chat, fs: __('replayed_no_media'));
        }
        ...
        switch ($to_edit) {
            ...
            case 'size':
                ...
                if ($media instanceof Media\Photo) {
                    if (!preg_match("/^(\d+)\*(\d+)$/sui", $new_val, $matches))
                        $error_fe = __('change.image_size_bad');
                    if (!in_array($media->fileExt, ['.jpg', '.png']))
                        $error_fe = __('change.picture_format_not_support');
                    if (!extension_loaded('gd'))
                        $error_fe = __('change.gd_not_supported');
                    if (isset($error_fe))
                        return $this->answer($chat, fe: $error_fe, message_to_edit: $message_to_edit ?? $message);
    ...
    private function answer(int|string $peer, string $fs = null, string $fe = null, Message $message_to_edit = null, int|string|Message $reply2id = null): bool
    {
        ...
        if (!empty($fe)) {
            if ($message_to_edit->out or $message_to_edit->senderId == $this->getSelf()['id']) {
                ...
                $message_to_edit->reply($fe, parseMode: Constants::DefaultParseMode, noWebpage: true);
            }
            return true;
        }
        return false;
    ...
    public function globalAddsCommand(Message $message): bool
    {
        ...
        switch ($message_text) {
            case '/start':
                $fe = __('start_message', ['counter' => Helpers::formatSeconds((time() - $this->start_time))]);
                break;
            case '/usage':
                $fe = __('memory_usage', [
                    'usage'           => round(memory_get_usage() / 1024 / 1024, 2),
                    'real_usage'      => round(memory_get_usage(true) / 1024 / 1024, 2),
                    'peak_usage'      => round(memory_get_peak_usage() / 1024 / 1024, 2),
                    'real_peak_usage' => round(memory_get_peak_usage(true) / 1024 / 1024, 2),
                ]);
                break;
            case '/getmessage':
                if (($reply = $message->getReply()) !== null)
